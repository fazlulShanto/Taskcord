...
import { type UserServerData } from './interfaces';
...
export const InviteBot: FC<InviteBotProps> = ({ onNext }) => {
  ...
  const renderStepHeader = () => {
    return (
      <div className="bg-background sticky -top-0.5 z-10 p-3">
        <h2 className="text-primary text-xl font-bold">Bring Task Management to Discord</h2>
            ...
            <CommandList>
              ...
              <CommandGroup>
                {data?.map((server) => (
                  <CommandItem
                    key={server?.id}
                    className="group border-border flex w-full gap-4 rounded-none border-b py-2 data-[selected='true']:bg-gray-950"
                    data-selected={'false'}
                    onSelect={() => {
      ...
      <Card key={selectedServer?.id} className="mx-auto w-[70%]">
        <CardContent className="flex h-full flex-col justify-between p-0">
          <div className="flex flex-grow gap-3 p-6">
            ...
            />
            <div className="text-md text-primary flex flex-col gap-1.5 font-normal">
              <p className="font-bold"> {selectedServer?.name} </p>
              <p className="text-muted-foreground text-sm">ID : {selectedServer?.id}</p>
    ...
    <div className="relative flex flex-col gap-3">
      ...
      <div className="flex w-full justify-end px-6">
        <Button
          ...
          className={cn(
            'text-secondary border hover:text-gray-700 disabled:pointer-events-auto disabled:opacity-80',
            {
